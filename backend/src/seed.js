...
async function seed() {
  ...
  const hostelsData = HOSTEL_NAMES.map(([name, type], i) => {
    ...
    const avgCap =
      name.includes("Sapphire") || name.includes("Emerald") || name.includes("Pearl")
        ? 4
        : name.includes("Coral")
          ? 1.5
          : name.includes("Ruby")
  ...
  const studentsData = Array.from({ length: 500 }, (_, i) => {
    ...
    const name =
      i === 0 ? "Gowtham Kumar" : `${pick(gender === "Male" ? FIRST_M : FIRST_F)} ${pick(LAST)}`;
    const department = pick(DEPARTMENTS);
    const year = i === 0 ? 3 : pick([1, 2, 3, 4]);
    const batch = 2026 - year;
    const regNo =
      i === 0 ? "7376242AD142" : `${batch}${String(int(100, 999))}${String(i).padStart(3, "0")}`;
    const hostel = pick(
      hostelsData.filter((h) => h.type === (gender === "Male" ? "Boys" : "Girls")),
    );
    ...
    return {
      ...
      email:
        i === 0
          ? "gowtham.aids@bitsathy.ac.in"
          : `${name.split(" ")[0].toLowerCase()}.${DEPT_SHORT[department].toLowerCase()}${year}${i}@bitsathy.ac.in`,
  ...
  const notificationsData = Array.from({ length: 100 }, (_, i) => {
    ...
    const bodies = {
      Academic: [
        "Semester exam timetable published on the student portal.",
        "Internal assessment marks are now available.",
      ],
      Hostel: [
        "Room inspection scheduled this weekend across all blocks.",
        "Water supply maintenance from 10 AM to 1 PM.",
      ],
      Mess: [
        "Special dinner menu announced for the weekend.",
        "Mess bill for the month has been finalised.",
      ],
      Payment: [
        "Hostel fee second instalment due next week.",
        "Payment receipt generated for your last transaction.",
      ],
      Event: [
        "Inter-hostel cricket tournament registrations are open.",
        "Cultural night at the open-air auditorium.",
      ],
    };
    return {
