...
router.post("/apply", async (req, res) => {
  try {
    ...
    if (!regNo || !hostelId) {
      return res
        .status(400)
        .json({ error: "Registration number and hostel selection are required." });
    }
    ...
    res
      .status(201)
      .json({ message: "Hostel application submitted successfully", application: newApp });
  } catch (error) {
...
router.post("/room-change", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Room change request submitted successfully", request: newRequest });
  } catch (error) {
