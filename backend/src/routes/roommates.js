...
// Calculate roommate compatibility score based on department, hometown, habits
...
router.get("/matches", async (req, res) => {
  try {
    ...
    // Fetch existing roommate requests
    ...
    existingRequests.forEach((req) => {
      const otherReg =
        req.requesterRegNo === currentStudent?.regNo ? req.targetRegNo : req.requesterRegNo;
...
router.post("/request", async (req, res) => {
  try {
    ...
    if (!requesterRegNo || !targetRegNo) {
      return res
        .status(400)
        .json({ error: "Requester and target registration numbers are required." });
    }
