...
function Page() {
  return (
    <AppShell title="Hostel application" breadcrumb={["Apply"]}>
      <form
        onSubmit={(e) => { e.preventDefault(); toast.success("Application submitted", { description: "Track the status from your dashboard." }); }}
        ...
        <div className="space-y-5">
          <section className="rounded-2xl border bg-card p-6 shadow-soft">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              {[["Full name", currentStudent.name], ["Register number", currentStudent.regNo], ["Department", currentStudent.department], ["Year", String(currentStudent.year)], ["Gender", currentStudent.gender], ["Mobile number", currentStudent.mobile]].map(([l, v]) => (
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2 sm:col-span-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {["Wi-Fi", "Study table", "Attached bathroom", "Hot water", "Power backup", "Ground floor"].map((f) => (
                    <label key={f} className="flex cursor-pointer items-center gap-2 rounded-xl border px-3 py-2 text-sm hover:border-primary/40">
        ...
        <div className="space-y-5">
          <section className="rounded-2xl border bg-card p-6 shadow-soft">
            ...
            <div className="mt-4 space-y-3">
              {["Aadhaar card", "Community certificate", "Income certificate", "Passport size photo"].map((d) => (
                <label key={d} className="flex cursor-pointer items-center justify-between rounded-xl border border-dashed px-4 py-3 text-sm hover:border-primary/50">
                  <span>{d}</span>
