  ...
  return (
    <AppShell title="Room change request" breadcrumb={["Room change"]}>
      <div className="grid gap-5 lg:grid-cols-[1.4fr_1fr]">
        <form
          onSubmit={(e) => { e.preventDefault(); toast.success("Room change request submitted"); }}
        ...
        <section className="rounded-2xl border bg-card p-6 shadow-soft">
          ...
          <ol className="mt-5 space-y-6 border-l pl-5">
            {[["Submitted", "31 Jul 2026", true], ["Warden review", "In progress", true], ["Admin approval", "Pending", false], ["Room allotted", "Pending", false]].map(([s, d, active]) => (
              <li key={s as string} className="relative">
                <span className={`absolute -left-[26px] top-1 size-3 rounded-full ring-4 ring-card ${active ? "bg-primary" : "bg-muted-foreground/40"}`} />
                <p className="text-sm font-semibold">{s as string}</p>
