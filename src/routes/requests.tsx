  ...
  return (
    <AppShell title="Roommate requests" breadcrumb={["Requests"]}>
      <div className="grid gap-5 lg:grid-cols-2">
        {[["Incoming requests", incoming, true], ["Outgoing requests", outgoing, false]].map(([label, list, inc]) => (
          <section key={label as string} className="rounded-2xl border bg-card p-5 shadow-soft">
            <h2 className="text-lg font-bold">{label as string}</h2>
            <ul className="mt-4 space-y-3">
              {(list as typeof matches).map((m) => (
                <li key={m.matchId} className="flex flex-wrap items-center gap-3 rounded-xl border p-3">
