...
import { BedDouble, Bell, CalendarCheck, CheckCircle2, ClipboardList, CreditCard, Heart, Home, Wrench, } from "lucide-react";
...
export const Route = createFileRoute("/dashboard")({
    head: () => ({
        meta: [
            ...
            {
                name: "description",
                content: "Track your hostel status, allocated room, pending requests, attendance and fee dues in one student dashboard.",
            },
...
function Dashboard() {
    ...
    return (<AppShell title={`Welcome back, ${currentStudent.name.split(" ")[0]}`} breadcrumb={["Dashboard"]} actions={<>
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border bg-card p-5 shadow-soft">
            ...
            borderRadius: 12,
                  ...
                  <Area type="monotone" dataKey="applications" stroke="var(--color-chart-1)" fill="url(#apps)" strokeWidth={2}/>
                  <Area type="monotone" dataKey="approved" stroke="var(--color-chart-2)" fill="url(#appr)" strokeWidth={2}/>
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {shortcuts.map((s, i) => (<motion.div key={s.to} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                <Link to={s.to} className="group flex items-center gap-3 rounded-2xl border bg-card p-4 shadow-soft transition-all hover:-translate-y-1 hover:shadow-elegant">
        ...
        <div className="space-y-5">
          <div className="rounded-2xl border bg-card p-5 shadow-soft">
            <div className="flex items-center gap-4">
              <img src={currentStudent.avatar} alt={currentStudent.name} className="size-16 rounded-2xl object-cover"/>
              <div>
          ...
          <div className="rounded-2xl border bg-card p-5 shadow-soft">
            ...
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={statusPie} dataKey="value" innerRadius={45} outerRadius={70} paddingAngle={4}>
          ...
          <div className="rounded-2xl border bg-card p-5 shadow-soft">
            ...
            <ul className="mt-4 space-y-3">
              {notifications.slice(0, 4).map((n) => (<li key={n.id} className="flex gap-3">
          ...
          <div className="rounded-2xl border bg-card p-5 shadow-soft">
            ...
            <ol className="mt-4 space-y-4 border-l pl-4">
              {activity.map(([text, when]) => (<li key={text} className="relative">
