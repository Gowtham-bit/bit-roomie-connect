...
import { Eye, EyeOff, GraduationCap, Loader2, Shield, UserCog } from "lucide-react";
...
export const Route = createFileRoute("/login")({
    head: () => ({
        meta: [
            ...
            {
                name: "description",
                content: "Sign in to the BIT hostel portal as a student, warden or admin using your register number.",
            },
...
function LoginPage() {
    ...
    const { register, handleSubmit, formState: { errors, isSubmitting }, } = useForm({ defaultValues: { regNo: "", password: "", remember: true } });
    ...
    return (<div className="gradient-hero relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-14">
      ...
      <motion.div initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="glass relative w-full max-w-md rounded-3xl p-7 shadow-elegant sm:p-9">
        ...
        <div className="mt-6 grid grid-cols-3 gap-2 rounded-2xl bg-muted/70 p-1.5">
          {roles.map((r) => (<button key={r.key} type="button" onClick={() => setRole(r.key)} aria-pressed={role === r.key} className={cn("flex flex-col items-center gap-1 rounded-xl px-2 py-2.5 text-xs font-semibold transition-all", role === r.key
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4" noValidate>
          <div className="space-y-2">
            ...
            <Input id="regNo" placeholder={role === "student" ? "7376242AD142" : "BIT-STAFF-1042"} aria-invalid={!!errors.regNo} {...register("regNo", {
        ...
        minLength: { value: 6, message: "Enter at least 6 characters" },
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input id="password" type={show ? "text" : "password"} placeholder="••••••••" className="pr-11" aria-invalid={!!errors.password} {...register("password", {
        required: "Password is required",
        minLength: { value: 6, message: "Minimum 6 characters" },
    })}/>
              <button type="button" onClick={() => setShow((s) => !s)} aria-label={show ? "Hide password" : "Show password"} className="absolute inset-y-0 right-3 flex items-center text-muted-foreground hover:text-foreground">
