...
function HostelsPage() {
  ...
  const [type, setType] = useState<"All" | "Boys" | "Girls">("All");
  const [sort, setSort] = useState("availability");

  const list = useMemo(() => {
    let l = hostels.filter(
      (h) =>
        h.name.toLowerCase().includes(q.toLowerCase()) && (type === "All" || h.type === type),
    );
    l = [...l].sort((a, b) =>
      sort === "availability"
        ...
        {list.map((h, i) => {
          ...
          return (
            <motion.article
              ...
              className="group overflow-hidden rounded-2xl border bg-card shadow-soft transition-all hover:-translate-y-1 hover:shadow-elegant"
            >
