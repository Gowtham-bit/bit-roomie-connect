...
function Page() {
    ...
    return (<AppShell title="Fee payment" breadcrumb={["Payments"]} actions={<Button variant="hero" onClick={() => toast.success("Redirecting to payment gateway")}>Pay now</Button>}>
      <div className="mb-5 grid gap-4 sm:grid-cols-3">
        {[["Pending amount", pending], ["Paid this year", paid], ["Transactions", mine.length]].map(([l, v], i) => (<div key={l} className="rounded-2xl border bg-card p-5 shadow-soft">
            <p className="text-sm text-muted-foreground">{l}</p>
            <p className="font-display mt-2 text-3xl font-extrabold">{i === 2 ? v : `₹${v.toLocaleString("en-IN")}`}</p>
          </div>))}
      </div>
      <section className="overflow-x-auto rounded-2xl border bg-card shadow-soft">
        <table className="w-full text-sm">
          ...
          <tbody>
            {mine.map((p) => (<tr key={p.id} className="border-t">
