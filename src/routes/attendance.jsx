...
function Page() {
    ...
    return (<AppShell title="Attendance" breadcrumb={["Attendance"]} actions={<Button variant="outline" onClick={() => toast.success("Report downloaded")}>Download report</Button>}>
      <div className="grid gap-5 lg:grid-cols-[1.5fr_1fr]">
        <section className="rounded-2xl border bg-card p-6 shadow-soft">
          ...
          <div className="mt-2 grid grid-cols-7 gap-2">
            {days.map((d, i) => (<div key={d.id} className={`flex aspect-square flex-col items-center justify-center rounded-xl text-sm font-semibold ${d.status === "Present" ? "bg-accent/15 text-accent" : d.status === "Absent" ? "bg-destructive/10 text-destructive" : "bg-warning/15 text-warning"}`}>
                {i + 1}
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[["Present", count("Present")], ["Absent", count("Absent")], ["Leave", count("Leave")]].map(([l, v]) => (<div key={l} className="rounded-2xl border bg-card p-4 text-center shadow-soft">
