...
function Page() {
    ...
    return (<AppShell title="Complaint management" breadcrumb={["Complaints"]}>
      <div className="grid gap-5 lg:grid-cols-[1fr_1.3fr]">
        <form onSubmit={(e) => { e.preventDefault(); toast.success("Complaint registered", { description: "You will be notified when staff is assigned." }); }} className="space-y-4 rounded-2xl border bg-card p-6 shadow-soft">
          <h2 className="text-lg font-bold">Raise a complaint</h2>
          <div className="space-y-2">
            <Label>Category</Label>
            <div className="grid grid-cols-3 gap-2">
              {["Electricity", "Water", "Internet", "Furniture", "Cleaning", "Others"].map((c) => (<label key={c} className="cursor-pointer rounded-xl border px-2 py-2 text-center text-xs font-medium hover:border-primary/50">
        ...
        <section className="rounded-2xl border bg-card p-5 shadow-soft">
          ...
          <ul className="mt-4 space-y-3">
            {mine.map((c) => (<li key={c.id} className="rounded-xl border p-4">
