...
import { Bell, BedDouble, Building2, CalendarCheck, ClipboardList, CreditCard, FileBarChart, Heart, LayoutDashboard, LogOut, Menu, MessageSquare, Repeat, Search, Settings, Users, UserCircle, Wrench, X, } from "lucide-react";
...
export function AppShell({ role = "student", title, breadcrumb, actions, children, }) {
        ...
        {nav.map((section) => (<div key={section.group}>
            ...
            <div className="space-y-1">
              {section.items.map((item) => {
                ...
                return (<Link key={item.to} to={item.to} onClick={() => setOpen(false)} className={cn("group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all", active
                        ? "bg-primary/10 text-primary"
                        : "text-muted-foreground hover:bg-muted hover:text-foreground")}>
                    {active && (<motion.span layoutId="nav-active" className="gradient-primary absolute left-0 h-6 w-1 rounded-r-full"/>)}
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <AnimatePresence>
        {open && (<>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setOpen(false)} className="fixed inset-0 z-40 bg-foreground/40 backdrop-blur-sm lg:hidden"/>
            <motion.aside initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }} transition={{ type: "spring", damping: 26, stiffness: 240 }} className="fixed inset-y-0 left-0 z-50 w-64 border-r bg-sidebar lg:hidden">
              <Button variant="ghost" size="icon" aria-label="Close menu" className="absolute top-4 right-2" onClick={() => setOpen(false)}>
                <X className="size-4"/>
              </Button>
              {sidebar}
            </motion.aside>
      ...
      <div className="lg:pl-64">
        <header className="glass sticky top-0 z-30 border-b px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="relative hidden max-w-sm flex-1 md:block">
              ...
              <Input placeholder="Search students, rooms, complaints…" className="pl-9" aria-label="Global search"/>
            </div>
            ...
            <div className="ml-auto flex items-center gap-1">
              ...
              <Button asChild variant="ghost" size="icon" className="relative" aria-label="Notifications">
                <Link to="/notifications">
                  ...
                  {unread > 0 && (<span className="absolute top-1.5 right-1.5 flex size-4 items-center justify-center rounded-full bg-destructive text-[9px] font-bold text-destructive-foreground">
                      {unread > 9 ? "9+" : unread}
                    </span>)}
                </Link>
              </Button>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="ml-1 flex items-center gap-2 rounded-full border p-1 pr-3 transition-colors hover:bg-muted" aria-label="Account menu">
        ...
        <main className="px-4 py-6 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="mx-auto max-w-7xl">
            <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
              <div>
                <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  {(breadcrumb ?? [title]).map((b) => (<span key={b} className="flex items-center gap-1.5">
                      <span>/</span>
...
export function StatCard({ icon: Icon, label, value, trend, tone = "primary", }) {
    ...
    return (<motion.div whileHover={{ y: -4 }} className="rounded-2xl border bg-card p-5 shadow-soft transition-shadow hover:shadow-elegant">
      <div className="flex items-start justify-between">
        <span className={cn("flex size-11 items-center justify-center rounded-xl", tones[tone])}>
          <Icon className="size-5"/>
        </span>
        {trend && (<Badge variant="secondary" className="rounded-full text-[11px]">
            {trend}
          </Badge>)}
      </div>
